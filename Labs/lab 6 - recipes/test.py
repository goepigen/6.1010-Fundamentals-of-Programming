...
def test_compound_possibilities():
    ...
    # just for testing
    recipes = [
        ("compound", "cookie", [("chocolate cookie", 1)]),
    ...
    check_compound_possibilities(
        lab.compound_ingredient_possibilities(recipes), expected
    )
...
def test_add_recipes_random():
    for num_ingredients in (20, 100, 1000):
        ...
        totals = {
            str(i): random.randint(1, max_quantity) for i in range(num_ingredients)
        }
...
def test_cheapest_forbidden_small():
    ...
    assert lab.cheapest_flat_recipe(
        example_recipes_db, "cheese", ("milking stool",)
    ...
    assert lab.cheapest_flat_recipe(
        example_recipes_db, "burger", ("vinegar", "milk")
    ) == {
        "yeast": 2,
        "salt": 2,
        "flour": 4,
...
def test_combine_recipes_big():
    ...
    for title, inp, outp in cases:
        assert canonize_flat_recipes(lab.combine_recipes(inp)) == canonize_flat_recipes(
            outp
        ), f"failed big {title}"
