...
def test_backwards_stereo():
    input = {"rate": 20, "right": [1, 2, 3, 4, 5], "left": [4, 5, 6, 7, 8]}

    input_copy = copy.deepcopy(input)

    expected = {"rate": 20, "right": [5, 4, 3, 2, 1], "left": [8, 7, 6, 5, 4]}

    compare_sounds(lab.backwards_stereo(input_copy), expected)

    assert input == input_copy, "lab.backwards should not mutate the sound input"


def test_mix_stereo():
    sound1 = {"rate": 20, "right": [1, 2, 3, 4, 5], "left": [4, 5, 6, 7, 8]}
    sound2 = {"rate": 20, "right": [0, 10, 11, 0, 8], "left": [9, 1, 3, 2, 1]}
    weight_sound1 = 0.2

    mixed_right = [0.2, 8.4, 9.4, 0.8, 7.4]
    mixed_left = [8.0, 1.8, 3.6000000000000005, 3.0, 2.4000000000000004]

    expected_mixed_stereo_sound = {
        "rate": 20,
        "right": mixed_right,
        "left": mixed_left,
    }
    ...
    assert (
        lab.mix_stereo(sound1, sound2, weight_sound1) == expected_mixed_stereo_sound
    ), "Not the same"
