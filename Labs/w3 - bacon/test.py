...
def test_acted_together_tiny():
    ...
    did_act_together = lab.acted_together(db_tiny, actor_id_1, actor_id_2)

    assert (
        did_act_together is True
    ), f"Expected actors {actor_id_1} and {actor_id_2} acted together to be true but received false."
...
def test_tiny_bacon_number_0():
    bacon_number = 0
    result = lab.actors_with_bacon_number(db_tiny, bacon_number)

    expected = {4724}

    assert (
        result == expected
    ), f"Expected {len(expected)} actors with bacon number {bacon_number} but got {len(result)}"
...
def test_actor_to_actor_path_tiny():
    ...
    assert result == expected, f"Expected path {expected} but got {result}"
